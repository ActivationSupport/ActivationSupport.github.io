  ...
  try {
    ...
    if (apptUrls.bookingUrl) {
      sh.getRange(r, 1, 1, 4).setValues([[
        ...
        "",
        ""
      ]]).setFontSize(10).setFontColor("#00A8E0").setBackground("#EFF6FF");
      if (apptUrls.bookingUrl) sh.getRange(r, 1).setFormula('=HYPERLINK("' + apptUrls.bookingUrl + '","📅 Book an Appointment")');
      if (apptUrls.adminUrl)   sh.getRange(r, 2).setFormula('=HYPERLINK("' + apptUrls.adminUrl   + '","🔧 Admin Dashboard")');
      sh.getRange(r, 1, 1, 4).setFontWeight("bold");
      sh.setRowHeight(r, 22);
      r++;
    }
...
function processOrderLogCsv(text) {
  ...
  const cDtrStatus = col("DTR Status (enriched)");
  const cProduct   = col("Product Type (Broken out lvl 3)");
  ...
  const cShipDate   = col("Ship Date (SP)");
  ...
  for (var i = 1; i < matrix.length; i++) {
    ...
    // Use DTR Status (enriched) as the primary status; fall back to Order Status
    // only if DTR Status is blank for this line. Normalize both to canonical names.
    var status    = normalizeStatus(dtrStatus || orderStatus);
    var product   = get(cProduct);
